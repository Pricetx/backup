...
if [ "$1" == "--remote" ]; then
    ...
    cat "${SCRIPTDIR}"/backup.cfg /tmp/hostname "${SCRIPTDIR}"/"${SCRIPTNAME}" | ssh -T -p ${REMOTEPORT} ${REMOTEUSER}@${REMOTESERVER}
...
||||||| parent of 6f4ba41... Fixed tab/space width
        #Script has been called with invalid flags
        echo "Usage: $0 [--remote]"
        exit
fi
=======
	...
	echo "Usage: $0 [--remote]"
	exit
||||||| parent of 4529d0e... Actually fixed tab/space width
	#Script has been called with invalid flags
	echo "Usage: $0 [--remote]"
	exit
=======
    ...
    echo "Usage: $0 [--remote]"
    exit
>>>>>>> 4529d0e... Actually fixed tab/space width
fi
>>>>>>> 6f4ba41... Fixed tab/space width
